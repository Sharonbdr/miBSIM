...
if __name__ == '__main__':
    ...
    parser.add_option("--skip_mir", dest="SKIP", help="skip miRNA feature generation (assumes miRNA feature files have been already created)",
                      default=False)
    parser.add_option("--remove_files", dest="RMDIR", help="remove feature sub process files after biochemical+ prediciton",
                      default=False)
    ...
    if options.TRANSCRIPTS is None:
        options.TRANSCRIPTS=os.getcwd()+'/Data/genes.txt'
    ...
    if not options.SKIP:
        ...
        for i in range(10):
            get_SA_bg(SEQUENCE_FILE = utldir + 'canon_'+options.MIRNAME+'_%d.txt'%i, TEMP_FOLDER = temp_folder,
                      OUTFILE = utldir + 'canon_'+options.MIRNAME+'_%d_bg_vals.txt'%i, VIEPATH = vienna_path)
    ...
    mycall = [vienna_path + 'RNAplfold', '-L', '40', '-W', '80', '-u', '15']
