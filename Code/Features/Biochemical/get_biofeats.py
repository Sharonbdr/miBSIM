...
def main_prep():
    ...
    od = str(Path(os.getcwd()).parents[2]) + '/Output/bio_output/'
    md = od + 'util_files/'
    ...
    # Create miR files folder
    mirdir = str(Path(os.getcwd()).parents[2]) +  '/miR_DataFiles/'
    if (not os.path.isdir(mirdir)):
        os.makedirs(mirdir)

    return od, md, scdir, mirdir
...
if __name__ == '__main__':
    ...
    outdir, utldir, temp_folder, mir_folder = main_prep()
    ...
    if not options.SKIP:
        ...
        gen12.call_kds(MIRNAME = options.MIRNAME, MIRSEQ = options.MIRSEQ, MIRLEN = 10, LOAD_MODEL = "trained_model/model-100",
                 OUTFILE = mir_folder+options.MIRNAME+'_kds.txt')
        print('kds')
        os.chdir('../rnaplfold')
