...
def call_kds(MIRNAME, MIRSEQ, MIRLEN, LOAD_MODEL, OUTFILE, MIRDATA=None, PASSENGER=None):

    if MIRSEQ is not None:
        calculate_12mer_kds(MIRNAME, MIRSEQ, MIRLEN, LOAD_MODEL, OUTFILE)

    else:
        mirdata = pd.read_csv(MIRDATA, sep='\t', index_col='mir')
        if MIRNAME == 'all':
            for row in mirdata.iterrows():
                mirname, mirseq = row[0], row[1]['guide_seq']
                print(mirname, mirseq)
                calculate_12mer_kds(mirname, mirseq, MIRLEN,
                    LOAD_MODEL, OUTFILE.replace('MIR', mirname))
